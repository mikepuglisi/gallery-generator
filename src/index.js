...
(async () => {
  const sourcePathInput = await prompts({
    ...
    name: 'value',
    default: "./",
    message: 'Paste the directory of the full size images you wish to convert to a gallery. (Default: Current Directory)',
  ...
  const relativeBasePathInput  = await prompts({
    ...
    name: 'value',
    default: "./",
    message: 'Relative Path from which images are served (Default: ./assets/gallery)',
    // validate: value => value < 18 ? `Nightclub is 18+ only` : true
  });
